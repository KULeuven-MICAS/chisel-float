...
// Implement non-IEEE 754 compliant floating point multiplication, resulting in a non-IEEE 754 compliant floating point.

module fp_mul_alt_to_alt #(
  ...
  always_comb begin : norm_shift_amount
    // Map values that would become subnormal to zero (non-IEEE754 compliant behavior).
  ...
  // Classification before round. In non-IEEE output formats, max exponent is still a finite value.
  assign of_before_round = final_exponent > 2 ** (EXP_BITS_C) - 1;
  assign uf_before_round = final_exponent == 0;  // exponent for subnormals capped to 0

  // Assemble result before rounding.
  ...
  // fpnew_rounding performs modulo addition on a fixed width. If rounding causes a carry out of
  // the AbsWidth field, the value wraps (e.g. max + 1 -> 0). Detect that wrap to saturate.
  assign of_after_round = (rounded_abs < pre_round_abs);

  // ------
  // Result
  // ------
  logic [WIDTH_out-1:0] regular_result;
  assign regular_result = (of_before_round || of_after_round)
      ? {pre_round_sign, {(WIDTH_out - 1) {1'b1}}}
      : {rounded_sign, rounded_abs};
  assign result_o       = regular_result;
